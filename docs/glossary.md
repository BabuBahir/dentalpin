...
| MCP | Model Context Protocol. The `mcp` module mounts a streamable-HTTP MCP server at `/api/v1/mcp/` re-exposing curated registry tools (read via `patients:read`, create via `patients:write`) behind `dp_` tokens. See `docs/technical/mcp/overview.md`, ADR 0033. |
